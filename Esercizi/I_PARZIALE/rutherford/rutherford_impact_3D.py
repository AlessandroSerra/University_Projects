...
def init_values(N_particles, N_steps, Ze, Zo, energy, alpha_mass):
    ...
    for k in range(N_particles):
        ...
        pos_z = 100 * (2 * np.random.rand() - 1) * impact_parameter
...
pos_array, vel_array, theta_list, impact_parameter = run_rutherford_exp3D(N_particles, N_steps, Ze, Zo, energy, alpha_mass, tau)
...
if what_to_do == 't':
    ...
    ax.set_title('Esperimento di Rutherford in 3 dimensioni')
    ax.set_xlabel('x (d)')
    ax.set_ylabel('y (d)')
    ax.set_zlabel('z (d)')
